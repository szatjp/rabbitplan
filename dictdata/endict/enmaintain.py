...
from dictdata.models import EnWord,En2Cn,Ja2En

class EnWordLi(ListView):
    paginate_by = 10
    ...
    def get_queryset(self):
        ...
        words = EnWord.objects.values('fwordno','fword','fpronunciation','fwordclass').all().order_by('fwordno')
        return words
...
class EnWordCreate(CreateView):
    model = EnWord
    fields = ['fword','fpronunciation','fwordclass']
    template_name = 'dictedit/worddet.html'
    def get_context_data(self, **kwargs):
        context = super(EnWordCreate, self).get_context_data(**kwargs)
        return context
